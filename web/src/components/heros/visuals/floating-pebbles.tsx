...
export default function FloatingPebbles() {
  ...
  return (
    <div className="relative w-[500px] h-[500px] flex items-center justify-center">
      ...
      <div className="absolute z-10 w-24 h-24 rounded-full bg-white shadow-[0_0_40px_rgba(109,79,199,0.5)] flex items-center justify-center p-2">
        <img src="/zenauraa logo main.png" alt="ZenAuraa" className="w-full h-full object-contain drop-shadow-md" />
      </div>
      ...
      {MODALITIES.map((mod, i) => {
        ...
        const radius = 150 + Math.sin(i * 45) * 50; 
        ...
        return (
          ...
          >
            ...
            <span className="mt-2 text-xs font-semibold text-[#1E2059] bg-white/50 backdrop-blur-sm px-2 py-0.5 rounded-full shadow-sm whitespace-nowrap">
              {mod.name}
            </span>
          </div>
        );
      ...
      <style jsx>{`
        @keyframes float {
          0% { transform: translate(var(--x, 0), calc(var(--y, 0) - 5px)) rotate(-5deg); }
          100% { transform: translate(var(--x, 0), calc(var(--y, 0) + 5px)) rotate(5deg); }
        }
      `}</style>
    </div>
  );
