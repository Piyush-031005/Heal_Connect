...
export default function ZodiacWheel() {
  ...
  return (
    <div className="relative w-[600px] h-[600px] md:w-[800px] md:h-[800px] flex items-center justify-center">
      ...
      <div className="absolute inset-0 bg-gradient-to-tr from-primary/10 dark:from-[#1a1235]/40 via-transparent to-accent/10 rounded-full blur-[120px] pointer-events-none" />
      ...
      >
        <svg viewBox="0 0 800 800" className="w-full h-full overflow-visible">
          ...
          {/* Outer glowing halo */}
          <circle cx={cx} cy={cy} r={radius + 40} className="stroke-primary" opacity="0.1" strokeWidth="1" fill="none" />
          <circle cx={cx} cy={cy} r={radius + 30} className="stroke-primary" opacity="0.3" strokeWidth="0.5" strokeDasharray="2 6" fill="none" />
          
          {/* Main Track */}
          <circle cx={cx} cy={cy} r={radius} className="stroke-primary" opacity="0.4" strokeWidth="1.5" fill="none" />
          <circle cx={cx} cy={cy} r={radius - 10} className="stroke-primary" opacity="0.15" strokeWidth="1" fill="none" />
          
          {/* Inner complex rings */}
          <circle cx={cx} cy={cy} r={radius - 60} className="stroke-primary" opacity="0.3" strokeWidth="1" strokeDasharray="8 12" fill="none" />
          <circle cx={cx} cy={cy} r={radius - 80} className="stroke-accent" opacity="0.2" strokeWidth="2" strokeDasharray="1 10" fill="none" />
          <circle cx={cx} cy={cy} r={radius - 120} className="stroke-primary" opacity="0.4" strokeWidth="1" fill="none" />
            ...
            return (
              ...
              >
                ...
                <g style={{ animation: 'spin 120s linear infinite reverse', animationPlayState: playState }}>
                  ...
                  <circle cx="0" cy="0" r="32" className={`transition-all duration-300 backdrop-blur-md ${isHovered ? "fill-primary/15 stroke-primary" : "fill-foreground/5 dark:fill-black/60 stroke-primary/40"}`} strokeWidth="1" />
                  <circle cx="0" cy="0" r="28" fill="transparent" className={`transition-all duration-300 ${isHovered ? "stroke-accent" : "stroke-accent/30"}`} strokeWidth="0.5" strokeDasharray="2 4" />
                  ...
                  <g className="transition-all duration-500 pointer-events-none" style={{ transform: isHovered ? 'scale(1.15)' : 'scale(1)' }}>
                    <image 
                      ...
                      width="160" 
                      height="160" 
                      className="transition-all duration-500 dark:mix-blend-screen mix-blend-multiply dark:invert-0 invert dark:hue-rotate-0 hue-rotate-[180deg]" 
                      style={{ 
      ...
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0">
        <div className="w-48 h-48 rounded-full flex items-center justify-center relative">
          ...
          <Image src="/logo.png" alt="HealConnect" width={140} height={140} quality={100} unoptimized className="rounded-full animate-pulse dark:mix-blend-screen mix-blend-multiply dark:invert-0 invert dark:hue-rotate-0 hue-rotate-[180deg] drop-shadow-[0_0_20px_rgba(214,180,107,0.9)]" style={{ animationDuration: '3s' }} />
