    ...
    <div className="min-h-screen flex flex-col md:flex-row font-sans relative overflow-hidden" style={{background:'#0F0723',color:'#fff'}}>
      ...
      <div className="hidden md:flex flex-col justify-between w-1/2 p-12 bg-white/5 backdrop-blur-md relative z-10 border-r border-white/20">
        <div className="relative z-10">
          ...
          <div className="space-y-8">
            <div className="flex items-center gap-5">
              <div className="w-12 h-12 rounded-full bg-white/20 border border-white/30 flex items-center justify-center shadow-[0_0_15px_rgba(46,196,182,0.2)]">
            ...
            <div className="flex items-center gap-5">
              <div className="w-12 h-12 rounded-full bg-white/20 border border-white/30 flex items-center justify-center shadow-[0_0_15px_rgba(214,180,107,0.2)]">
        ...
        <div className="relative z-10 mt-auto pt-12 border-t border-white/20">
          <p className="text-white/60 text-sm tracking-wider uppercase">ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã†â€™ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â© 2026 Tara Infotech. All rights reserved.</p>
        </div>
      </div>

      {/* Right ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Â ÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Â¦Ãƒâ€šÃ‚Â¡ÃƒÆ’Ã†â€™Ãƒâ€ Ã¢â‚¬â„¢ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â‚¬Å¡Ã‚Â¬Ãƒâ€¦Ã‚Â¡ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã†â€™Ãƒâ€šÃ‚Â¢ÃƒÆ’Ã‚Â¢ÃƒÂ¢Ã¢â€šÂ¬Ã…Â¡Ãƒâ€šÃ‚Â¬ÃƒÆ’Ã¢â‚¬Å¡Ãƒâ€šÃ‚Â Form */}
          ...
          <CardContent className="space-y-6 pt-6">
              ...
              <div className="flex rounded-xl border border-white/20 overflow-hidden bg-black/5 dark:bg-black/50 p-1 gap-1">
                <button type="button" onClick={() => { setRole('user'); setError(''); }}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg text-sm font-semibold transition-all duration-300 ${
                    role === 'user' ? 'bg-[#7C3AED] text-white shadow-lg' : 'text-white/60 hover:text-white hover:bg-black/5 dark:hover:bg-white/5'}`}>
                  <User className="w-4 h-4" /> User
                </button>
                <button type="button" onClick={() => { setRole('expert'); setError(''); }}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 rounded-lg text-sm font-semibold transition-all duration-300 ${
                    role === 'expert' ? 'bg-[#7C3AED] text-white shadow-lg' : 'text-white/60 hover:text-white hover:bg-black/5 dark:hover:bg-white/5'}`}>
                  <Sparkles className="w-4 h-4" /> Expert
                </button>
              </div>
            )}

            {error && <div className="rounded-lg bg-red-900/30 border border-red-500/50 px-4 py-3 text-sm text-red-400 flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-red-500 animate-pulse"/>{error}</div>}
            {success && <div className="rounded-lg bg-green-900/30 border border-green-500/50 px-4 py-3 text-sm text-green-400 flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-green-500 animate-pulse"/>{success}</div>}
              ...
              <form onSubmit={handleLogin} className="space-y-5">
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Mail className="absolute left-3 top-3 h-5 w-5 text-white/60 group-focus-within:text-accent transition-colors" />
                    <Input type="email" required placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)}
                      className="pl-10 py-6 bg-white/10 backdrop-blur-sm border-white/20 text-white placeholder:text-white/60 focus-visible:ring-accent focus-visible:border-accent rounded-xl" />
                  </div>
                </div>
                <div className="space-y-2">
                  ...
                  <div className="relative group">
                    <Lock className="absolute left-3 top-3 h-5 w-5 text-white/60 group-focus-within:text-accent transition-colors" />
                    <Input type={showPassword ? 'text' : 'password'} required placeholder="Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢Ã¢â‚¬Â¢" value={password} onChange={e => setPassword(e.target.value)} className="pl-10 pr-10 py-6 bg-white/10 backdrop-blur-sm border-white/20 text-white placeholder:text-white/40 focus-visible:ring-purple-400 focus-visible:border-purple-400 rounded-xl" />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-white/60 hover:text-white transition-colors">
            ...
            {mode === 'forgot' && (
              <form onSubmit={handleForgotPassword} className="space-y-5">
                ...
                <button type="button" onClick={() => { setMode('login'); setError(''); setSuccess(''); }} className="w-full text-center text-sm text-white/60 hover:text-white transition-colors">
                  ? Back to login
                </button>
              </form>
            )}
            ...
            {mode === 'login' && (
              <>
                <div className="relative flex items-center py-2">
                  ...
                  <span className="flex-shrink-0 mx-4 text-white/60 text-xs uppercase tracking-widest font-semibold">Or continue with</span>
                  <div className="flex-grow border-t border-white/20" />
                </div>
                <div className="space-y-3">
                  <Button type="button" variant="outline" onClick={handleGoogleSignIn} className="w-full py-6 bg-white/10 backdrop-blur-sm border-white/20 hover:bg-white/10 backdrop-blur-sm border-white/20 text-white shadow-sm transition-all rounded-xl">
                  ...
                  <Button type="button" variant="outline" onClick={handleAppleSignIn} className="w-full py-6 bg-white/10 backdrop-blur-sm border-white/20 hover:bg-white/10 backdrop-blur-sm border-white/20 text-white shadow-sm transition-all rounded-xl">
