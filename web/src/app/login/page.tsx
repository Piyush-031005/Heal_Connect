      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-6 md:p-12 relative z-10">
        ...
        <Card className="w-full max-w-md bg-card/80 dark:bg-card/80 backdrop-blur-xl border border-border shadow-2xl rounded-2xl overflow-hidden">
          <CardHeader className="space-y-2 pb-6 border-b border-border bg-black/5 dark:bg-white/5">
            <CardTitle className="text-2xl font-bold text-[#2A1658] tracking-wide">
              {mode === 'login' ? 'Log in to your account' : 'Reset your password'}
            </CardTitle>
            <CardDescription className="text-[#2A1658]/70 text-base">
          ...
          <CardContent className="space-y-6 pt-6">
            ...
            {mode === 'login' && (
              <form onSubmit={handleLogin} className="space-y-5">
                <div className="space-y-2">
                  <Label className="text-[#2A1658]">Email address</Label>
                  <div className="relative group">
                    <Mail className="absolute left-3 top-3 h-5 w-5 text-[#2A1658]/70 group-focus-within:text-accent transition-colors" />
                    <Input type="email" required placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)}
                      className="pl-10 py-6 bg-white/40 backdrop-blur-md border-[#2A1658]/10 border-[#2A1658]/20 text-[#2A1658] placeholder:text-[#2A1658]/70 focus-visible:ring-accent focus-visible:border-accent rounded-xl" />
                  </div>
                </div>
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <Label className="text-[#2A1658]">Password</Label>
                  ...
                  <div className="relative group">
                    <Lock className="absolute left-3 top-3 h-5 w-5 text-[#2A1658]/70 group-focus-within:text-accent transition-colors" />
                    <Input type={showPassword ? 'text' : 'password'} required placeholder="Ã‚â€¢Ã‚â€¢Ã‚â€¢Ã‚â€¢Ã‚â€¢Ã‚â€¢Ã‚â€¢Ã‚â€¢" value={password} onChange={e => setPassword(e.target.value)}
                      className="pl-10 pr-10 py-6 bg-white/40 backdrop-blur-md border-[#2A1658]/10 border-[#2A1658]/20 text-[#2A1658] placeholder:text-[#2A1658]/70 focus-visible:ring-accent focus-visible:border-accent rounded-xl" />
                    <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3 text-[#2A1658]/70 hover:text-[#2A1658] transition-colors">
            ...
            {mode === 'forgot' && (
              <form onSubmit={handleForgotPassword} className="space-y-5">
                ...
                <button type="button" onClick={() => { setMode('login'); setError(''); setSuccess(''); }} className="w-full text-center text-sm text-[#2A1658]/70 hover:text-[#2A1658] transition-colors">
                  ? Back to login
                </button>
              </form>
            )}
            ...
            {mode === 'login' && (
              <>
                ...
                <div className="space-y-3">
                  <Button type="button" variant="outline" onClick={handleGoogleSignIn} className="w-full py-6 bg-white/40 backdrop-blur-md border-[#2A1658]/10 border-[#2A1658]/20 hover:bg-white/40 backdrop-blur-md border-[#2A1658]/10 text-[#2A1658] shadow-sm transition-all rounded-xl">
                  ...
                  <Button type="button" variant="outline" onClick={handleAppleSignIn} className="w-full py-6 bg-white/40 backdrop-blur-md border-[#2A1658]/10 border-[#2A1658]/20 hover:bg-white/40 backdrop-blur-md border-[#2A1658]/10 text-[#2A1658] shadow-sm transition-all rounded-xl">
                ...
                <p className="text-center text-sm text-[#2A1658]/70 pt-4">
                  ...
                  <Link href="/signup" className="text-primary font-semibold hover:text-[#2A1658] transition-colors hover:underline">Sign up</Link>
                </p>
              </>
            )}
          </CardContent>
