...
function SectionLabel({ num, title, subtitle }: { num: string; title: string; subtitle?: string }) {
  return (
    <div className="flex items-start gap-3 mb-4">
      <span className="w-7 h-7 rounded-full bg-indigo-100 text-indigo-600 text-xs font-bold flex items-center justify-center flex-shrink-0 mt-0.5">{num}</span>
      <div>
...
const selectCls = "w-full h-12 rounded-xl border border-indigo-200 bg-[#f5f3ff] px-4 text-sm text-gray-800 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition appearance-none";
const textareaCls = "w-full rounded-xl border border-indigo-200 bg-[#f5f3ff] px-4 py-3 text-sm text-gray-800 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-400 focus:border-transparent transition resize-none";

export default function ExpertOnboardingProfilePage() {
  ...
  useEffect(() => {
    ...
    if (!token) { router.replace('/expert/login'); return; }
    astrologerApi.getApplication(token).then((res) => {
      if (!res.success) { astrologerTokenStore.clear(); router.replace('/expert/login'); return; }
      const p = res.data?.profile;
      if (p) {
        if (p.applicationStatus === 'APPROVED' && p.accountStatus === 'ACTIVE') { router.replace('/expert/dashboard'); return; }
        if (['ADMIN_REVIEW', 'UNDER_REVIEW', 'PENDING_REVIEW', 'SUBMITTED', 'PROFILE_COMPLETED'].includes(p.applicationStatus)) { router.replace('/expert/onboarding/submitted'); return; }
        if (p.specializations?.length) set('selectedAreas', p.specializations);
        if (p.professionalBio) set('practiceBio', p.professionalBio);
      }
      setLoading(false);
    }).catch(() => router.replace('/expert/login'));
  }, [router]);
  ...
  const handleNext = async () => {
    ...
    if (!form.whyHealConnect.trim()) { setError('Please tell us what interests you about ZenAuraa.'); return; }
    ...
    try {
      ...
      router.push('/expert/onboarding/verification');
  ...
  return (
    <div className="min-h-screen bg-[#f5f3ff] flex flex-col md:flex-row font-sans">
      ...
      <div className="hidden md:flex flex-col justify-between w-5/12 p-12 bg-gradient-to-br from-indigo-500 via-indigo-600 to-purple-700 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <Link href="/" className="flex items-center gap-2 mb-16">
            <Image src="/center_logo_final.png" alt="ZenAuraa" width={36} height={36} className="rounded-full" />
      ...
      <div className="flex-1 flex flex-col items-center justify-start px-6 py-12 overflow-y-auto">
        ...
        <div className="w-full max-w-lg">
          ...
          <div className="bg-white rounded-2xl shadow-xl border border-indigo-100 p-8 space-y-8">
            <div>
            ...
            <div className="flex justify-between items-center pt-2">
              <button onClick={() => router.push('/expert/onboarding')}
