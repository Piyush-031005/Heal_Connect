      ...
      <main className="flex-1 pt-32 pb-24">
        <div className="container mx-auto px-4 max-w-4xl">
          ...
          <div className="text-center mb-16">
            ...
            <h1 className="text-4xl md:text-5xl font-extrabold text-indigo-950 mb-4 tracking-tight">
                    ...
                    <button
                      ...
                      className={`px-5 py-2.5 rounded-full text-sm font-semibold transition-all ${
                        activeCategory === cat
                          ...
                          : 'bg-white text-purple-700 hover:bg-indigo-50 border border-violet-200 hover:border-indigo-200 hover:text-indigo-600'
                      }`}
                    >
                      {cat as string}
                    </button>
                ...
                {filteredFaqs.map((faq) => (
                  <div 
                    ...
                    className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${
                      openFaq === faq.id ? 'border-indigo-200 shadow-md ring-1 ring-indigo-100' : 'border-purple-100 shadow-sm hover:border-indigo-100 hover:shadow'
                    }`}
                  >
                    <button
                    ...
                    >
                      <h3 className={`font-semibold text-lg transition-colors pr-8 ${openFaq === faq.id ? 'text-indigo-700' : 'text-indigo-950'}`}>
                        {faq.question}
                      </h3>
                      <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 ${openFaq === faq.id ? 'bg-indigo-100 text-indigo-600 rotate-180' : 'bg-purple-50 text-purple-400'}`}>
                    ...
                    <AnimatePresence>
                      {openFaq === faq.id && (
                        ...
                        >
                          <div className="px-6 pb-6 pt-2 text-purple-700 leading-relaxed border-t border-purple-50">
                            {faq.answer}
                          </div>
                        </motion.div>
                      )}
          ...
          <div className="mt-16 bg-indigo-50 rounded-3xl p-8 md:p-12 text-center border border-indigo-100">
            <h3 className="text-2xl font-bold text-indigo-950 mb-4">Still have questions?</h3>
