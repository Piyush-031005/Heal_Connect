            ...
            <div className="flex flex-col sm:flex-row items-start gap-6">
              ...
              <div className="flex-1 min-w-0 text-center sm:text-left space-y-2">
                ...
                {p.languages.length > 0 && (
                  <p className="text-xs text-purple-500 pt-0.5">🌐 Spoken Languages: <span className="font-semibold text-purple-800">{p.languages.join(', ')}</span></p>
                )}
              </div>
            </div>

            {p.bio && (
              <p className="text-sm text-purple-700 mt-5 pt-4 border-t border-purple-100 leading-relaxed bg-purple-50/30 p-4 rounded-2xl border border-indigo-100/50">
                {p.bio}
              </p>
            )}
                    ...
                    <div className="flex items-center justify-between mb-2">
                      ...
                      <div className="flex items-center gap-0.5">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <Star key={i} className={`w-3.5 h-3.5 ${i < r.rating ? 'text-purple-300 fill-purple-300' : 'text-purple-200'}`} />
                        ))}
                      </div>
                    </div>
