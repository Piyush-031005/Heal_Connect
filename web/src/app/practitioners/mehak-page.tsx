    ...
    try {
      ...
      if (res.success && res.data?.session) {
        ...
        const errorDetail = res.errors?.length
          ? res.errors.map((e) => e.message).join(' â”¬â•– ')
        ...
        <Card className=•bg-white border border-yellow-100/80 shadow-md rounded-3xl overflow-hidden hover:shadow-lg transition-all duration-300•>
          <CardContent className=•p-6 md:p-8•>
              ...
              <div className=•flex-1 min-w-0 text-center sm:text-left space-y-2•>
                <div className=•flex flex-col sm:flex-row sm:items-center justify-between gap-2•>
                  <div>
                    ...
                    <p className=•text-[#4f46e5] font-semibold text-sm mt-0.5•>{p.specialties.join(' â”¬â•– ')}</p>
                  </div>
                  {p.isVerified && (
                ...
                {p.languages.length > 0 && (
                  <p className=•text-xs text-gray-500 pt-0.5•>â‰¡Æ’Ã®Ã‰ Spoken Languages: <span className=•font-semibold text-gray-700•>{p.languages.join(', ')}</span></p>
                )}
              </div>
            </div>
            ...
            <div className=•flex items-center justify-between mt-6 pt-5 border-t border-yellow-100•>
              <div>
                <span className=•text-3xl font-extrabold text-[#1a1a1a]•>Î“Ã©â•£{p.perMinuteRate}</span>
