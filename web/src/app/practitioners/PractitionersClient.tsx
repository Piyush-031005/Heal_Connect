...
export default function PractitionersPage() {
  ...
  return (
    <div className=•min-h-screen flex flex-col font-sans• style={{background:•linear-gradient(145deg, #F5F0FF 0%, #EDE9FE 20%, #DDD6FE 45%, #C4B5FD 70%, #A5B4FC 100%)•}}>
      <div style={{position:•absolute•,top:•-10%•,left:•-5%•,width:•40%•,height:•40%•,borderRadius:•50%•,background:•radial-gradient(circle, rgba(167â‚¹39,250,0.35) 0%, transparent 70%)•,filter:•blur(80px)•,pointerEvents:•none•}} />
      <div style={{position:•absolute•,bottom:•-10%•,right:•-5%•,width:•50%•,height:•50%•,borderRadius:•50%•,background:•radial-gradient(circle, rgba(129â‚¹40,248,0.3) 0%, transparent 70%)•,filter:•blur(100px)•,pointerEvents:•none•}} />
      <Navbar />
      ...
      <main className=•flex-1 container mx-auto px-4 pt-24 pb-8•>
                ...
                {suggestions.map((p) => (
                  ...
                  >
                    ...
                    <div className=•min-w-0 flex-1•>
                      ...
                      <p className=•text-xs text-[#6d28d9]/70 truncate•>{p.specialties.slice(0, 2).join(' Ã‚Â· ') || 'â€â€'}</p>
                    </div>
          ...
          <div className=•mb-6 p-5 rounded-2xl bg-white/80 backdrop-blur-sm border border-yellow-100 shadow-sm grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4•>
            ...
            <div>
              ...
              <select value={filters.minRating} onChange={(e) => setFilters((f) => ({ ...f, minRating: e.target.value }))} className={SELECT_CLS}>
                ...
                {['3', '3.5', '4', '4.5'].map((r) => <option key={r} value={r}>Ã¢Â­Â {r}+</option>)}
              </select>
            </div>
            <div>
              <label className=•text-xs text-[#6d28d9]/70 mb-1 block•>Max Ã¢â€šÂ¹/min</label>
        ...
        {activeFilterCount > 0 && (
          <div className=•flex flex-wrap gap-2 mb-5•>
            ...
            {filters.minRating && <Badge variant=•outline• className=•border-yellow-300 text-[#4338ca] bg-yellow-50 gap-1•>Ã¢Â­Â {filters.minRating}+<button onClick={() => setFilters((f) => ({ ...f, minRating: '' }))}><X className=•h-3 w-3• /></button></Badge>}
            {filters.maxRate && <Badge variant=•outline• className=•border-yellow-300 text-[#4338ca] bg-yellow-50 gap-1•>Ã¢â€°Â¤ Ã¢â€šÂ¹{filters.maxRate}/min<button onClick={() => setFilters((f) => ({ ...f, maxRate: '' }))}><X className=•h-3 w-3• /></button></Badge>}
        ...
        <div className=•pt-10 px-5 pb-5 flex flex-col flex-1•>
          <div className=•flex items-start justify-between mb-1•>
            <div>
              ...
              <p className=•text-sm text-indigo-600 font-medium•>{p.specialties.slice(0, 2).join(' Ã‚Â· ') || 'â€â€'}</p>
            </div>
          ...
          <div className=•flex items-center gap-3 mt-2 mb-3•>
            <div className=•flex items-center gap-1•>
              ...
              <span className=•text-sm font-semibold text-[#2d1b69]•>{p.avgRating || 'â€â€'}</span>
            ...
            <div className=•flex items-center gap-1•>
              ...
              <span className=•text-xs text-[#6d28d9]/70 truncate max-w-[80px]•>{p.languages.slice(0, 2).join(', ') || 'â€â€'}</span>
            </div>
          </div>

          {/* Bio â€â€ always takes up space even if empty */}
          ...
          <div className=•flex items-center justify-between pt-3 mt-3 border-t border-purple-100•>
            <div>
              <span className=•text-lg font-bold text-[#2d1b69]•>Ã¢â€šÂ¹{p.perMinuteRate}</span>
