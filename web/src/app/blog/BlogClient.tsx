    ...
    <div className="min-h-screen bg-[#faf9f6] text-indigo-950 flex flex-col font-sans">
      ...
      <main className="flex-grow container mx-auto px-4 py-12 max-w-6xl space-y-16">
        ...
        <section className="space-y-6">
          <div className="flex items-center justify-between gap-4 border-b border-indigo-200/80 pb-6 overflow-x-auto scrollbar-none">
            ...
            {isPractitioner && (
              <button onClick={openWriteForm}
                className="shrink-0 flex items-center gap-1.5 bg-indigo-500 hover:bg-indigo-600 text-white font-extrabold text-xs px-4 py-2 rounded-full transition-all shadow-md">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {trendingArticles.map((art) => (
                <motion.div key={art.id} whileHover={{ y: -4 }} onClick={() => setSelectedArticle(art)}
                  className="bg-white border border-indigo-100 hover:border-indigo-300 rounded-3xl overflow-hidden shadow-md hover:shadow-xl transition-all cursor-pointer flex flex-col justify-between">
                  <div className="relative h-56 w-full overflow-hidden">
                    ...
                    <span className="absolute top-4 left-4 bg-indigo-500 text-white text-[11px] font-extrabold uppercase px-3 py-1 rounded-full shadow-md">
                      {art.category}
                    </span>
                  </div>
            ...
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-6">
              {filteredArticles.map((art) => (
                <motion.div key={art.id} whileHover={{ y: -4 }}
                  className="bg-white border border-indigo-100 hover:border-indigo-300 rounded-3xl overflow-hidden shadow-sm hover:shadow-lg transition-all flex flex-col justify-between relative">
                  ...
                  {isPractitioner && art.practitionerPost && (
                    <div className="absolute top-3 right-3 z-10 flex gap-1.5">
                      <button onClick={(e) => { e.stopPropagation(); openEditForm(art); }}
                        className="p-1.5 bg-white/90 backdrop-blur-sm rounded-lg border border-indigo-200 text-indigo-600 hover:bg-indigo-50 shadow-sm transition-colors" title="Edit post">
                  ...
                  <div onClick={() => setSelectedArticle(art)} className="cursor-pointer">
                    <div className="relative h-44 w-full overflow-hidden">
                      <img src={art.image} alt={art.title} className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                      <span className="absolute top-3 left-3 bg-indigo-500 text-white text-[10px] font-extrabold uppercase px-2.5 py-0.5 rounded-full">
                        {art.category}
                      </span>
        ...
        <section className="rounded-3xl bg-gradient-to-br from-indigo-500 via-purple-500 to-indigo-600 p-8 md:p-12 text-center text-white relative overflow-hidden shadow-xl space-y-6">
          ...
          <div className="max-w-md mx-auto relative z-10">
            {isSubscribed ? (
              <div className="p-4 rounded-full bg-white text-indigo-800 font-extrabold text-sm flex items-center justify-center gap-2 shadow-lg">
              ...
              {isPractitioner && selectedArticle.practitionerPost && (
                <div className="absolute top-4 left-4 z-20 flex gap-2">
                  <button onClick={() => openEditForm(selectedArticle)}
                    className="flex items-center gap-1.5 bg-white text-indigo-700 text-xs font-extrabold px-3 py-1.5 rounded-xl shadow-md border border-indigo-200 hover:bg-indigo-50 transition-colors">
