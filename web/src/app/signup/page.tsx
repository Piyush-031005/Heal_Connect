...
export default function SignupPage() {
  ...
  function handleGoogleSignIn() {
    ...
    const stateParam = role === 'expert' ? 'expert' : 'user';
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center p-6 md:p-12 relative z-10">
        ...
        <Card className="w-full max-w-md bg-card/80 dark:bg-card/80 backdrop-blur-xl border border-purple-200 shadow-2xl rounded-2xl overflow-hidden">
          <CardHeader className="space-y-2 pb-6 border-b border-purple-200 bg-white/50 border-b border-purple-200">
            <CardTitle className="text-2xl font-bold text-black tracking-wide">Create your account</CardTitle>
          ...
          <CardContent className="space-y-6 pt-6">
            ...
            <form onSubmit={handleRegister} className="space-y-5">
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="name" type="text" placeholder="John Doe" value={name} onChange={(e) => setName(e.target.value)} required autoComplete="name" 
                    className="pl-10 h-12 border-purple-200 focus-visible:ring-accent focus-visible:border-accent bg-white/40 backdrop-blur-md text-black placeholder:text-[#4c1d95]/70 transition-all" />
                </div>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  ...
                  <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="Min. 8 chars, 1 uppercase, 1 number" value={password} onChange={(e) => setPassword(e.target.value)} required autoComplete="new-password" 
                    className="pl-10 pr-10 h-12 border-purple-200 focus-visible:ring-accent focus-visible:border-accent bg-white/40 backdrop-blur-md text-black placeholder:text-[#4c1d95]/70 transition-all" />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-3.5 text-[#4c1d95]/70 hover:text-black transition-colors" tabIndex={-1}>
              ...
              <Button type="submit" disabled={loading || !!success} className="w-full py-6 text-base font-bold rounded-md border-0 shadow-lg transition-all duration-300 bg-[#FAD058] hover:bg-[#F0C240] text-black">
            ...
            <div className="space-y-3">
              <Button type="button" variant="outline" onClick={() => window.location.href = '/login?mode=phone'} className="w-full h-12 bg-white/60 backdrop-blur-sm border-purple-200 hover:bg-accent/10 text-black shadow-sm transition-all rounded-xl">
                <Phone className="mr-3 h-5 w-5 text-current" />
                Continue with Phone
              </Button>
              <Button type="button" variant="outline" onClick={handleGoogleSignIn} className="w-full h-12 bg-white/60 backdrop-blur-sm border-purple-200 hover:bg-accent/10 text-black shadow-sm transition-all rounded-xl">
              ...
              <Button type="button" variant="outline" onClick={handleAppleSignIn} className="w-full h-12 bg-white/60 backdrop-blur-sm border-purple-200 hover:bg-accent/10 text-black shadow-sm transition-all rounded-xl">
