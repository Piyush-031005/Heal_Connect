  ...
  return (
    <AdminShell>
      <div className="space-y-6">
        <h1 className="text-2xl font-bold text-indigo-950 dark:text-white">Appointments</h1>
        ...
        <div className="bg-white dark:bg-indigo-900 rounded-xl shadow-sm border border-purple-100 dark:border-violet-700 overflow-hidden">
          <div className="p-4 border-b border-purple-100 dark:border-violet-700 flex flex-col sm:flex-row gap-4 justify-between items-center">
            ...
            <div className="flex gap-2">
              ...
              <input type="date" className="px-4 py-2 rounded-lg border border-violet-300 dark:border-violet-600 bg-white dark:bg-indigo-950 text-indigo-950 dark:text-white outline-none" />
            </div>
          </div>
              ...
              <tbody className="divide-y divide-purple-100 dark:divide-violet-700">
                {data.map(item => (
                  <motion.tr key={item.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-purple-50 dark:hover:bg-violet-700/50">
                    ...
                    <td className="px-6 py-4 text-purple-700 dark:text-purple-300">{item.type}</td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col text-sm">
                        <span className="flex items-center gap-1 text-indigo-950 dark:text-white"><CalendarIcon size={14}/> {item.date}</span>
                        <span className="flex items-center gap-1 text-purple-500"><Clock size={14}/> {item.time}</span>
                      </div>
                    </td>
